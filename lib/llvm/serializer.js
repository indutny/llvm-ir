...
class Serializer {
  ...
  data(str) {
    ...
    return `@${str.id} = internal unnamed_addr constant ${str.type.type} ` +
      `c"${bytes}"`;
  }

  array(arr) {
    const value = arr.value;
    const elems = value.elems.map(v => v.type.type + ' ' + v.value);
    return `@${arr.id} = internal unnamed_addr constant ${value.type.type} ` +
      `[ ${elems.join(', ')} ]`;
  }
  ...
  arg(instr, v) {
    ...
    if (v instanceof llvm.values.Array) {
      let out = '[ ';
      out += v.elems.map((elem) => {
        return this.arg(instr, elem.type) + ' ' + this.arg(instr, elem);
      }).join(', ');
      out += ' ]';
      return out;
    }
